...
def plot_1(plot_name, topologies=None):
    ...
    for topo in topologies:
        ...
        combination = pd.read_csv(
            f"benchmark/mincutsets/results/combination_matrix/{topo}_MCS_combination_matrix.csv"
        )
        minimalpaths = pd.read_csv(
            f"benchmark/pathsets/results/minimalpaths/{topo}_Pathsets_minimalpaths.csv"
        )
    ...
    ax1.bar(
        ...
        combination_times,
        width,
        label="Finding MCSs (combinatorial)",
        color="#D57E26",
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"plots/paper_faster_mincutset/{plot_name}.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def plot_2(plot_name, topologies=None):
    ...
    for topo in topologies:
        ...
        cnf = pd.read_csv(f"benchmark/mincutsets/results/cnf/{topo}_MCS_cnf.csv")
        shannon = pd.read_csv(
            f"benchmark/mincutsets/results/shannon/{topo}_MCS_shannon.csv"
        )
        # Sum the time columns
...
def plot_3(plot_name, topologies=None):
    ...
    ax1.set_xticks(x)
    # ax1.set_xticklabels(topologies, fontsize=FONT_SIZE - 2, rotation=ROTATION)
    labels = ax1.set_xticklabels(topologies, fontsize=FONT_SIZE - 2, rotation=ROTATION)
